...
from database_setup import Base, Shelter, Puppy, PuppyProfiles
...
#Add Shelters
shelter0 = Shelter(name = 'Null', maximum_capacity = 0, current_occupancy = 0)
session.add(shelter0)

shelter1 = Shelter(name = "Oakland Animal Services", address = "1101 29th Ave", city = "Oakland", state = "California", zipCode = "94601", website = "oaklandanimalservices.org", maximum_capacity = 5, current_occupancy = 0)
session.add(shelter1)

shelter2 = Shelter(name = "San Francisco SPCA Mission Adoption Center", address="250 Florida St", city="San Francisco", state="California", zipCode = "94103", website = "sfspca.org", maximum_capacity = 5, current_occupancy = 0)
session.add(shelter2)

shelter3 = Shelter(name = "Wonder Dog Rescue", address= "2926 16th Street", city = "San Francisco", state = "California" , zipCode = "94103", website = "http://wonderdogrescue.org", maximum_capacity = 7, current_occupancy = 0)
session.add(shelter3)

shelter4 = Shelter(name = "Humane Society of Alameda", address = "PO Box 1571" ,city = "Alameda" ,state = "California", zipCode = "94501", website = "hsalameda.org", maximum_capacity = 5, current_occupancy = 0)
session.add(shelter4)

shelter5 = Shelter(name = "Palo Alto Humane Society" ,address = "1149 Chestnut St." ,city = "Menlo Park", state = "California" ,zipCode = "94025", website = "paloaltohumane.org", maximum_capacity = 5, current_occupancy = 0)
...
def createPuppy(puppyName, puppyGender):
	new_puppy = Puppy(name = puppyName, gender = puppyGender, dateOfBirth = CreateRandomAge(),picture=random.choice(puppy_images) , shelter_id = None, weight= CreateRandomWeight())
	return new_puppy

def getShelter(shelterID):
	shelterToCheck = session.query(Shelter).filter_by(id = shelterID).one()
	return shelterToCheck

def insertPuppyIntoShelter(puppy, shelterID):
	shelter = getShelter(shelterID)
	print shelter.id
	shelter_capacity = shelter.maximum_capacity
	shelter_occupancy = shelter.current_occupancy
	if shelter_capacity == 0 or shelter_occupancy < shelter_capacity: #denotes infinite capacity
		shelter.current_occupancy += 1
		puppy.shelter_id = shelterID
		session.add(puppy)
		session.commit()
	else:
		print "This shelter is at capacity! No more puppies!"

def adoptPuppy(puppy):
	# decrement the shelter's occupancy
	shelterID = puppy.shelter_id
	shelter = getShelter(shelterID)
	shelter.current_occupancy -= 1
	session.add(shelter)
	session.commit()

	# set the puppy's shelter to 'adopted'
...
for i,x in enumerate(male_names):
	...
	# session.commit()
	shelterID = randint(1,6)
	newPuppy = createPuppy(x, 'male')
	insertPuppyIntoShelter(newPuppy, shelterID)
